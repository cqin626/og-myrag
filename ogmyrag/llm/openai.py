...
def get_openai_async_client_instance() -> AsyncOpenAI:
   ...
   if global_openai_async_client is None:
      try:
         ...
         app_logger.warning("OPENAI_API_KEY is not properly set!")
...
async def fetch_completion_openai(
   ...
   temperature: float | None,
    ...
    app_logger.debug(f"Sending query to {model} ...")
    app_logger.debug("Current conversation:\n\n" + "\n".join(
    ...
    try:
      ...
      app_logger.error(f"OpenAI API Connection Error: {e}")
      raise
    except RateLimitError as e:
      app_logger.error(f"OpenAI API Rate Limit Error: {e}")
      raise
    except APITimeoutError as e:
      app_logger.error(f"OpenAI API Timeout Error: {e}")
      raise
