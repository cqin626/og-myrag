    ...
    def insert_entities(self, entities: list[dict], label: str):
        ...
        try:
            with self.driver.session() as session:
                ...
                neo4j_logger.info(
                    f"Successfully inserted {len(entities)} entity(ies) with label {label}"
                )
        except Exception as e:
...
class AsyncNeo4jStorage:
    ...
    async def insert_entities(self, entities: list[dict], label: str):
        try:
            async with self.driver.session() as session:
                for entity in entities:
                    query = f"CREATE (n:{label} $props)"
                    await session.run(query, props=entity)
                neo4j_logger.info(
                    f"Successfully inserted {len(entities)} entity(ies) with label {label}"
                )
        except Exception as e:
